...
public class PetPanel extends JPanel {
    private PetAppGUI main;         //main GUI for access to user profile
    protected JLabel picture;       //picture of pet to be displayed

    // REQUIRES: mainUser must be instantiated
    // EFFECTS : main is set to mainImport; layout is set to BorderLayout;
    //           background is set and pet is added to panel
    public PetPanel(PetAppGUI mainImport) {
        main = mainImport;
    ...
    private void addPet() {
        try {
            Image img = ImageIO.read(new File("resources/"
                    + main.user.getPetType().toString().toLowerCase()
                    + "/pet.png"));
